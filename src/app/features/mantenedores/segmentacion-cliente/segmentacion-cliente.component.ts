...
export class SegmentacionClienteComponent implements OnInit {
  ...
  dataSource: SegmentacionCliente[] = []; // Ahora usa la interfaz Segmentacion cliente
  ...
  agregarServicio() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.segmentacionClienteService.crear(result).subscribe(
          (response) => {
  ...
  eliminarServicio(selectedItems: SegmentacionCliente[]) {
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc') {
    this.segmentacionClienteService.buscarFiltrado({
      pageNumber: this.pageNumber,
      pageSize: this.pageSize,
      sortField: sortField,
      sortDirection: sortDirection
    }).subscribe((data: PagedResponse<SegmentacionCliente[]>) => {
