...
export class ClasificacionClienteComponent implements OnInit {
  ...
  exportarExcel(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    const handleExport = (dataArray: any[], translationBase: string) => {
      ...
      if (translationBase === 'mantenedores.clasificacionCliente') {
    ...
    if (this.selectedData && Array.isArray(this.selectedData) && this.selectedData.length > 0) {
      ...
      handleExport(this.selectedData, 'mantenedores.clasificacionCliente');
      ...
      this.clasificacionClienteService.buscarFiltrado(filtros).subscribe(
        (response: any) => {
          ...
          handleExport(apiData, 'mantenedores.clasificacionCliente');
  ...
  onDeleteSingleSelected(id: string) {
    ...
    // Obtener las traducciones
    const titulo = this.translate.instant('alertas.eliminacionIndividualTitulo') + ' ' + this.translate.instant('mantenedores.clasificacionCliente.titulo');
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.clasificacionClienteService.borrar(Number(id)).subscribe({
          next: () => {
