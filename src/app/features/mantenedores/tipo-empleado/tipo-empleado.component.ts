...
export class TipoEmpleadoComponent implements OnInit {
  ...
  dataSource: TipoEmpleado[] = []; // Ahora usa la interfaz Tipo empleado
  ...
  agregarServicio() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.tipoEmpleadoService.crear(result).subscribe(
          (response) => {
  ...
  eliminarServicio(selectedItems: TipoEmpleado[]) {
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc') {
    this.tipoEmpleadoService.buscarFiltrado({
      pageNumber: this.pageNumber,
      pageSize: this.pageSize,
      sortField: sortField,
      sortDirection: sortDirection
    }).subscribe((data: PagedResponse<TipoEmpleado[]>) => {
