...
import { TipoClienteFormComponent } from "../../../shared/components/forms/tipo-cliente.component";
...
export class TipoClienteComponent implements OnInit {
  ...
  dataSource: TipoCliente[] = []; // Ahora usa la interfaz tipoCliente
  ...
  onViewSelected(id: string): void {
    const dialogRef = this.dialog.open(TipoClienteFormComponent, {
      width: '400px',
      data: {
        esActualizar: true,
  ...
  exportarExcel(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    const handleExport = (dataArray: any[], translationBase: string) => {
      ...
      if (translationBase === 'mantenedores.tipoCliente') {
    ...
    if (this.selectedData && Array.isArray(this.selectedData) && this.selectedData.length > 0) {
      ...
      handleExport(this.selectedData, 'mantenedores.tipoCliente');
      ...
      this.tipoClienteService.buscarFiltrado(filtros).subscribe(
        (response: any) => {
          ...
          handleExport(apiData, 'mantenedores.tipoCliente');
  ...
  eliminar(selectedItems: TipoCliente[]) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.tipoClienteService.borrarTodos(ids).subscribe({
          next: () => {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.tipoClienteService.borrar(Number(id)).subscribe({
          next: () => {
