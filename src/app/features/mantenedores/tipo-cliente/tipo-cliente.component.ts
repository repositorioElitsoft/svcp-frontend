...
export class TipoClienteComponent implements OnInit {
  ...
  onEditSelected(id: string) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        if (!result.tipoClienteDesc) {
          console.error("Descripción no válida:", result.tipoClienteDesc);
          return;
        }

        const formData: TipoCliente = {
          id: result.id,
          tipoClienteDesc: result.tipoClienteDesc
        };
  ...
  eliminarServicio(selectedItems: TipoCliente[]) {
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc') {
    this.tipoClienteService.buscarFiltrado({
      pageNumber: this.pageNumber,
      pageSize: this.pageSize,
      sortField: sortField,
      sortDirection: sortDirection
    }).subscribe((data: PagedResponse<TipoCliente[]>) => {
