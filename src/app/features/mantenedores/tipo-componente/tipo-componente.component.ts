...
import { TipoComponenteFormComponent } from "../../../shared/components/forms/tipo-componente.component";
@Component({
  ...
  styleUrl: "./tipo-componente.component.css",
})
export class TipoComponenteComponent implements OnInit {
  displayedColumns: string[] = []; // Se inicializa vacío
  dataSource: TipoComponente[] = []; // 
  titulo: string = 'Tipo Componente'; // Puedes cambiarlo dinámicamente
  hasSelection = false;
  ...
  onViewSelected(id: string): void {
    const dialogRef = this.dialog.open(TipoComponenteFormComponent, {
      width: '400px',
      data: {
        esActualizar: true,
  ...
  exportarExcel(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    const handleExport = (dataArray: any[], translationBase: string) => {
      ...
      if (translationBase === 'mantenedores.tipoComponente') {
    ...
    if (this.selectedData && Array.isArray(this.selectedData) && this.selectedData.length > 0) {
      ...
      handleExport(this.selectedData, 'mantenedores.tipoComponente');
      ...
      this.tipoComponenteService.buscarFiltrado(filtros).subscribe(
        (response: any) => {
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    ...
    this.tipoComponenteService.buscarFiltrado(mandatoryFilter).subscribe((data: PagedResponse<TipoComponente[]>) => {
  ...
  /*********************************** CRUD   - DELETE ***********************************/
  eliminar(selectedItems: TipoComponente[]) {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.tipoComponenteService.borrar(Number(id)).subscribe({
          next: () => {
