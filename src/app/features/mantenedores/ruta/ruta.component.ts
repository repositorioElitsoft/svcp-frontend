...
export class RutaComponent implements OnInit {
  ...
  exportarExcel(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    const handleExport = (dataArray: any[], translationBase: string) => {
      ...
      if (translationBase === 'mantenedores.ruta') {
    ...
    if (this.selectedData && Array.isArray(this.selectedData) && this.selectedData.length > 0) {
      ...
      handleExport(this.selectedData, 'mantenedores.ruta');
      ...
      this.rutaService.buscarFiltrado(filtros).subscribe(
        (response: any) => {
          ...
          handleExport(apiData, 'mantenedores.ruta');
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    ...
    this.rutaService.buscarFiltrado(mandatoryFilter).subscribe((data: PagedResponse<Rutas[]>) => {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    // Obtener las traducciones
    const titulo = this.translate.instant('alertas.eliminacionIndividualTitulo') + ' ' + this.translate.instant('mantenedores.ruta.titulo');
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.rutaService.borrar(Number(id)).subscribe({
          next: () => {
