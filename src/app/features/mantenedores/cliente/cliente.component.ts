...
import { ClienteFormComponent } from "../../../shared/components/forms/cliente.component";
...
export class ClienteComponent implements OnInit {
  ...
  dataSource: Cliente[] = []; // Ahora usa la interfaz cliente
  titulo: string = 'Cliente'; // Puedes cambiarlo dinámicamente
  hasSelection = false;
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    ...
    this.clienteService.buscarFiltrado(mandatoryFilter).subscribe((data: PagedResponse<Cliente[]>) => {
  ...
  /*********************************** CRUD   - DELETE ***********************************/
  eliminar(selectedItems: Cliente[]) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.clienteService.borrarTodos(ids).subscribe({
          next: () => {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    // Obtener las traducciones
    const titulo = this.translate.instant('alertas.eliminacionIndividualTitulo') + ' ' + this.translate.instant('mantenedores.cliente.titulo');
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.clienteService.borrar(Number(id)).subscribe({
          next: () => {
