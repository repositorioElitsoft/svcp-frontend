...
export class ZonaComponent implements OnInit {
  ...
  exportarExcel(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    const handleExport = (dataArray: any[], translationBase: string) => {
      ...
      if (translationBase === 'mantenedores.zona') {
    ...
    if (this.selectedData && Array.isArray(this.selectedData) && this.selectedData.length > 0) {
      ...
      handleExport(this.selectedData, 'mantenedores.zona');
      ...
      this.zonaService.buscarFiltrado(filtros).subscribe(
        (response: any) => {
          ...
          handleExport(apiData, 'mantenedores.zona');
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    ...
    this.zonaService.buscarFiltrado(mandatoryFilter).subscribe((data: PagedResponse<Zona[]>) => {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    // Obtener las traducciones
    const titulo = this.translate.instant('alertas.eliminacionIndividualTitulo') + ' ' + this.translate.instant('mantenedores.zona.titulo');
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.zonaService.borrar(Number(id)).subscribe({
          next: () => {
