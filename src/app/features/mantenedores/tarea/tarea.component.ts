...
import { TareaFormComponent } from "../../../shared/components/forms/tarea.component";
...
export class TareaComponent implements OnInit {
  ...
  dataSource: Tarea[] = []; // Ahora usa la interfaz tarea
  titulo: string = 'Tarea'; // Puedes cambiarlo dinámicamente
  hasSelection = false;
  ...
  exportarExcel(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    const handleExport = (dataArray: any[], translationBase: string) => {
      ...
      if (translationBase === 'mantenedores.tarea') {
    ...
    if (this.selectedData && Array.isArray(this.selectedData) && this.selectedData.length > 0) {
      ...
      handleExport(this.selectedData, 'mantenedores.tarea');
      ...
      this.tareaService.buscarFiltrado(filtros).subscribe(
        (response: any) => {
          ...
          handleExport(apiData, 'mantenedores.tarea');
  ...
  onFilterDeleted(filterData: { field: string, value: string }) {
    ...
    this.activeOptionalFilters.forEach((filter: { field: string, value: string }) => {
      if (filter.field === 'descripcionTarea') {
        newFilter.descripcionTarea = filter.value;
      }
    });
  ...
  eliminar(selectedItems: Tarea[]) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.tareaService.borrarTodos(ids).subscribe({
          next: () => {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.tareaService.borrar(Number(id)).subscribe({
          next: () => {
