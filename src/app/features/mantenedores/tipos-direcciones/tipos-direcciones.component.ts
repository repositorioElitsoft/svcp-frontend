...
import { TiposDireccionesFormComponent } from "../../../shared/components/forms/tipos-direcciones.component";
...
@Component({
  ...
  styleUrl: "./tipos-direcciones.component.css",
})
export class TiposDireccionesComponent implements OnInit {
  displayedColumns: string[] = []; // Se inicializa vacío
  dataSource: TiposDirecciones[] = []; // Ahora usa la interfaz tiposDirecciones
  titulo: string = 'Tipos direcciones'; // Puedes cambiarlo dinámicamente
  hasSelection = false;
  ...
  constructor(private cdr: ChangeDetectorRef,
    ...
    private tiposDireccionesService: TiposDireccionesService) { }
  ...
  onViewSelected(id: string): void {
    const dialogRef = this.dialog.open(TiposDireccionesFormComponent, {
      width: '400px',
      data: {
        esActualizar: true,
  ...
  obtenerDatos(sortField: string = 'id', sortDirection: string = 'asc', optionalFilter: any = {}) {
    ...
    this.tiposDireccionesService.buscarFiltrado(mandatoryFilter).subscribe((data: PagedResponse<TiposDirecciones[]>) => {
  ...
  /*********************************** CRUD   - DELETE ***********************************/
  eliminar(selectedItems: TiposDirecciones[]) {
  ...
  onDeleteSingleSelected(id: string) {
    ...
    // Obtener las traducciones
    const titulo = this.translate.instant('alertas.eliminacionIndividualTitulo') + ' ' + this.translate.instant('mantenedores.tiposDirecciones.titulo');
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        ...
        this.tiposDireccionesService.borrar(Number(id)).subscribe({
          next: () => {
